<template>
  <div class="intro-y flex items-center mt-8">
      <h2 class="text-lg font-medium mr-auto">Data list imageing</h2>
  </div>
  <div class="grid grid-cols-12 gap-6 mt-5 mb-20">
      <div class="intro-y col-span-12">
          <button class="btn btn-primary shadow-md mr-2" @click="router.push({ name: 'admin-image-create'})">
              Add new
          </button>
          <div class="intro-y box col-span-12 overflow-auto lg:overflow-visible">
              <table class="table table-report mt-2">
                  ...
                  <tbody v-for="(image, index) in images" :key="index">
                      <tr>
                          <td> {{ index +1 }}</td>
                          <td> {{ image.title }} </td>
                          <td>
                              <div class="flex justify-center items-center">
                                  <button class="flex items-center mr-3"
                                    @click="router.push({name: 'admin-image-edit', params: { id: image.id }})">
