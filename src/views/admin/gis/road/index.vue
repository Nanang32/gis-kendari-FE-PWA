<template>
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium capitalize mr-auto">Data Jalan</h2>
         <div class="intro-y flex  items-center mt-8">
    ...
    <div class="grid grid-cols-12 gap-6 mt-5 mb-20">
        <div class="intro-y col-span-12">
            <button class="btn btn-primary shadow-md mr-2" @click="router.push({name: 'admin-road-create'})">
                Tambah
            </button>
            <Export segment="roads" />
            <Import segment="roads" />
            <div class="intro-y col-span-12 overflow-x-auto">
                <table class="table table-report -mt-2">
                    ...
                    <tbody v-for="(road, index) in roads" :key="index">
                        <tr>
                            <td>{{ index + 1 }}</td>
                            <td>{{ road.Nm_Dat_Das }}</td>
                            <td>
                                <div class="flex justify-center items-center">
                                    <button class="flex items-center mr-3" @click="router.push({name: 'admin-road-edit', params: { id: road.id }})">
                                        <EditIcon class="w-4 h-4 mr-1" />
                                        Edit
                                    </button>
                                    <a class="flex items-center text-danger" href="javascript:;" @click="confirmDelete(road.id)">
...
async function search() {
    ...
    if ((response !== null) && (response.status === true)){
        roads.value = response.data.road.data
        lastPage.value = response.data.road.last_page
    }
    loading.value = false;
}
