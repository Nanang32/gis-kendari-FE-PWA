<template>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5 mb-20">
        <div class="intro-y col-span-12">
            <button class="btn btn-primary shadow-md mr-2" @click="router.push({name: 'admin-irrigation-create'})">
                Tambah
            </button>
            <Export segment="irrigations" />
            <Import segment="irrigations" />
            <div class="intro-y col-span-12 overflow-x-auto">
                <table class="table table-report -mt-2">
                    ...
                    <tbody v-for="(irrigation, index) in irrigations" :key="index">
                        <tr>
                            <td>{{ index + 1 }}</td>
                            <td>{{ irrigation.Nm_Dat_Das }}</td>
                            <td>
                                <div class="flex justify-center items-center">
                                    <button class="flex items-center mr-3" @click="router.push({name: 'admin-irrigation-edit', params: { id: irrigation.id }})">
                                        <EditIcon class="w-4 h-4 mr-1" />
                                        Edit
                                    </button>
                                    <a class="flex items-center text-danger" href="javascript:;" @click="confirmDelete(irrigation.id)">
