...
(function(dom) {
  ...
  function show(el) {
    if (!dom("[data-modal-replacer='" + dom(el).attr("id") + "']").length) {
      dom('<div data-modal-replacer="' + dom(el).attr("id") + '"></div>').insertAfter(el);
      dom(el).css({
        "margin-top": 0,
        "margin-left": 0
      });
      ...
      dom("body").css("padding-right", parseInt(dom("body").css("padding-right")) + getScrollbarWidth("html") + "px").addClass("overflow-y-hidden");
  ...
  function hide(el) {
    if (dom(el).hasClass("modal") && dom(el).hasClass("show")) {
      ...
      setTimeout(() => {
        ...
        dom('[data-modal-replacer="' + dom(el).attr("id") + '"]').replaceWith(el);
